...
package org.puremvc.java.multicore.demos.microservice.employeeadmin.user.view.components;
        ...
        switch (request.getPathInfo()) {
            case "/users":
                if(request.getMethod().equals("GET")) {
                    ...
                    response.setStatus(status);
                    response.setHeader("Location", "/users/" + ((Document)data).get("_id"));
                }
                break;

            default:
                Matcher matcher = Pattern.compile("/users/(.*)").matcher(request.getPathInfo()); // users/:id
                if (matcher.find()) {
                    if (request.getMethod().equals("GET")) {
                        response.setStatus(status);
                        responseWriter.write(((Document)data).toJson(writerSettings));
